...
class BC():
    ...
    def normalized_admittance(self, domain_index, normalized_admittance):
        ...
        if type(domain_index) == int:
            if type(normalized_admittance) == int or float:
                self.mu[domain_index] = np.ones_like(
                    self.AC.freq)*normalized_admittance/(self.AP.c0*self.AP.rho0)

        if type(domain_index) == list:
            for i in range(len(domain_index)):
                self.mu[domain_index[i]] = np.ones_like(
                    self.AC.freq)*normalized_admittance/(self.AP.c0*self.AP.rho0)
    ...
    def TMM(self, domain_index, TMM):
        ...
        mu_data = f_real(self.AC.freq).ravel() + 1j * \
            f_imag(self.AC.freq).ravel()
    ...
    def fluid(self, domain_index, cc, rhoc):
        ...
        self.rhoc[domain_index] = (np.ones_like(
            self.AC.freq, dtype=complex)*rhoc).ravel()
        self.cc[domain_index] = (np.ones_like(
            self.AC.freq, dtype=complex)*cc).ravel()
    ...
    def add_material(self, normal_incidence_alpha=[], statistical_alpha=[], octave_bands_statistical_alpha=[], octave_bands=[],
        ...
        if "parameters" in kwargs and "absorber_type" in kwargs:
            ...
            material = Material(freq_vec=self.AC.freq,
                                rho0=self.AP.rho0, c0=self.AP.c0)
            ...
            material = Material(normal_incidence_alpha=normal_incidence_alpha, statistical_alpha=statistical_alpha, octave_bands_statistical_alpha=octave_bands_statistical_alpha,
                                ...
                                surface_impedance=surface_impedance, freq_vec=self.AC.freq, rmk1=rmk1, rho0=self.AP.rho0, c0=self.AP.c0)
            ...
            if "absorber_type" in kwargs:
                if kwargs["absorber_type"] == "rigid":
                    ...
                    material.impedance_from_alpha(
                        absorber_type=kwargs["absorber_type"])
                    self.materials.append(material)
